...
**Full handoff for the next Grok session:**  
→ **[`docs/handoff/CONTINUE_NEXT_SESSION.md`](docs/handoff/CONTINUE_NEXT_SESSION.md)**  
(Task board snapshots also under `docs/handoff/`. Live board: [`PARITY_TASKS.md`](PARITY_TASKS.md).)
...
| **N** | PokéNav (party / dex / send / ride / badges) |
| **Space** (mounted flyer) | Hold to fly; release to fall |
| **Shift** (mounted water mon) | Hold to dive; release to surface (does not dismount) |
...
- [x] Faint mid-battle → switch; all faint → **blackout** (party soft-healed to **1 HP in place** — **no** bed/spawn teleport)
...
- [x] Pasture eggs + egg-move inheritance (E3)
- [x] Pasture ranch GUI (E4) — deposit/withdraw/breed
...
### Technical
- [x] Gilded chest models — simplified cube models (no UV bake failures)
- [x] Item model `particle` textures on ball/pokedex parents
- [ ] `@OnlyIn` warnings on client-only classes (harmless noise on NeoForge 26.2)
- [x] Spawn pressure tuned (softer caps / longer interval under load)

---

## What is MISSING (not really implemented)

- [x] Safe structure landmarks (feature-only; no unbound structure tags)
- [x] NPC trainers + dialogue + gym badges (v0)
- [x] Player↔player Pokémon trading UI + Link Cable trade evo
- [x] Eggs + egg moves from parents (daycare GUI still TODO)
- [x] Ride settings + flight/water mounts (v0; multi-seat/stats still TODO)
- [ ] Full ride multi-seat / MoLang physics
- [x] Double battles (v0 — `/doubleduel`)
- [ ] Shiny hunting systems / marks / ribbons gameplay
- [x] Showdown sim-authoritative turns when Graal protocol ready (native fallback)
...
## Blackout behaviour (canonical)

When the last party mon faints:

1. Battle ends (`LOST`)
2. Companion entity is recalled
3. **Every party mon** is set to **1 HP**, status cleared, a little PP restored
4. Player **stays where they are** (no teleport to bed/spawn)
5. Chat: blackout message + healing tip

This matches `BattleManager.handleBlackout` and `BATTLE.md`. Do not reintroduce bed-warp without an explicit design change.

---

## Full parity task board

See **[PARITY_TASKS.md](PARITY_TASKS.md)** for every remaining epic (Showdown engine, trainers, doubles, trade, eggs, ride, structures, mega/tera, shiny, cooking, etc.) with wave order and status.

### Recently landed (scaffolds / v0)
- Weather / terrain / hazards in wild battles
- Shiny 1/4096 + preserve on catch
- Pokemon eggs + pasture breeding → egg + egg-move inheritance (E3)
- Showdown line interpreter + choice slot → sim (B1.3 partial)
- Ride settings + flight / water mounts (R2/R3 v0)
- Trainer NPC (`/spawntrainer`) wild-style lead battle
- Trade UI (R on player) + Link Cable trade evolutions
...
| Items depth | ~50% |
| Social / story | ~15–20% |
| **Overall** | **~50–55% playable Cobblemon-like** |
